...
#     img_grads = filters.sobel(img_gray)
#     return img_grads
...
#         if s == 0:
#             return r, s
#         elif s == centers.shape[1]:
#             return r, s - 1
#         else:
#             x1, y1 = centers[r, s, 0], centers[r, s, 1]
#             x2, y2 = centers[r, s - 1, 0], centers[r, s - 1, 1]
#             d1 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x1, y1])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x1, y1]))
#             d2 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x2, y2])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x2, y2]))
#             if d1 > d2:
#                 return r, s - 1
#             else:
#                 return r, s
#     elif r == centers.shape[0]:
#         if s == 0:
#             return r - 1, s
#         elif s == centers.shape[1]:
#             return r - 1, s - 1
#         else:
#             x1, y1 = centers[r - 1, s, 0], centers[r - 1, s, 1]
#             x2, y2 = centers[r - 1, s - 1, 0], centers[r - 1, s - 1, 1]
#             d1 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x1, y1])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x1, y1]))
#             d2 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x2, y2])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x2, y2]))
...
#         if s == 0:
#             x1, y1 = centers[r - 1, s, 0], centers[r - 1, s, 1]
#             x2, y2 = centers[r, s, 0], centers[r, s, 1]
#             d1 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x1, y1])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x1, y1]))
#             d2 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x2, y2])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x2, y2]))
...
#                 return r - 1, s
#         elif s == centers.shape[1]:
#             x1, y1 = centers[r, s - 1, 0], centers[r, s - 1, 1]
#             x2, y2 = centers[r - 1, s - 1, 0], centers[r - 1, s - 1, 1]
#             d1 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x1, y1])) \
#                  + alpha * np.sum(np.square(xy_vectors[i, j] - xy_vectors[x1, y1]))
#             d2 = np.sum(np.square(lab_vectors[i, j] - lab_vectors[x2, y2])) \
...
#                 return r, s - 1
#         else:
#             close_centers = np.asarray([[r - 1, s - 1], [r - 1, s], [r, s], [r, s - 1]])
#             x = np.asarray([centers[r - 1, s - 1, 0], centers[r - 1, s, 0], centers[r, s, 0], centers[r, s - 1, 0]])
#             y = np.asarray([centers[r - 1, s - 1, 1], centers[r - 1, s, 1], centers[r, s, 1], centers[r, s - 1, 1]])
#             d = np.zeros(4)
#             lab_pixel = lab_vectors[i, j]
#             xy_pixel = xy_vectors[i, j]
#             for k in range(4):
#                 lab_center = lab_vectors[x[k], y[k]]
#                 xy_center = xy_vectors[x[k], y[k]]
#                 d[k] = np.sum(np.square(lab_pixel - lab_center)) + alpha * np.sum(np.square(xy_pixel - xy_center))
#             t = np.argmin(d)
#             return close_centers[t]
#
...
# def cluster_pixels(img, centers, lab_vectors, xy_vectors, alpha):
#     img_result = img.copy()
#     h, w = img.shape[0], img.shape[1]
#     r, s = 0, 0
#     a1, b1 = 0, 0
#     for i in range(h):
#         for j in range(w):
#             if s < centers.shape[1] and j > centers[min(r, centers.shape[0] - 1), s, 1]:  # update close cluster point
#                 s += 1
#             a2, b2 = find_closest_center2(i, j, centers, r, s, lab_vectors, xy_vectors, alpha)
#             if a2 != a1 or b2 != b1:
#                 img_result[i, j, :] = 0
#             a1, b1 = a2, b2
#         s = 0
